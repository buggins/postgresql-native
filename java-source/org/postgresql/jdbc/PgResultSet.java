...
public class PgResultSet implements ResultSet, org.postgresql.PGRefCursorResultSet {
  ...
  //JCP! if mvn.project.property.postgresql.jdbc.spec >= "JDBC4.2"
//JCP>   private LocalDateTime getLocalDateTime(int i) throws SQLException {
//JCP>     checkResultSet(i);
//JCP>     if (wasNullFlag) {
//JCP>       return null;
//JCP>     }
//JCP> 
//JCP>     int col = i - 1;
//JCP>     int oid = fields[col].getOID();
//JCP>     if (oid != Oid.TIMESTAMP) {
//JCP>       throw new PSQLException(
//JCP>               GT.tr("Cannot convert the column of type {0} to requested type {1}.",
//JCP>                   Oid.toString(oid), "timestamp"),
//JCP>               PSQLState.DATA_TYPE_MISMATCH);
//JCP>     }
//JCP>     if (isBinary(i)) {
//JCP>       TimeZone timeZone = getDefaultCalendar().getTimeZone();
//JCP>       return connection.getTimestampUtils().toLocalDateTimeBin(timeZone, this_row[col]);
//JCP>     }
//JCP> 
//JCP>     String string = getString(i);
//JCP>     return connection.getTimestampUtils().toLocalDateTime(string);
//JCP>   }
  //JCP! endif
  ...
  public <T> T getObject(int columnIndex, Class<T> type) throws SQLException {
    ...
    if (type == BigDecimal.class) {
      ...
      if (sqlType == Types.TIMESTAMP
              //JCP! if mvn.project.property.postgresql.jdbc.spec >= "JDBC4.2"
//JCP>               || sqlType == Types.TIMESTAMP_WITH_TIMEZONE
      //JCP! endif
      ) {
...
//JCP>               || sqlType == Types.TIMESTAMP_WITH_TIMEZONE
      ...
      // JSR-310 support
      //JCP! if mvn.project.property.postgresql.jdbc.spec >= "JDBC4.2"
//JCP>     } else if (type == LocalDate.class) {
//JCP>       if (sqlType == Types.DATE) {
//JCP>         Date dateValue = getDate(columnIndex);
//JCP>         if (wasNull()) {
//JCP>           return null;
//JCP>         }
//JCP>         return type.cast(dateValue.toLocalDate());
//JCP>       } else {
//JCP>         throw new SQLException("conversion to " + type + " from " + sqlType + " not supported");
//JCP>       }
//JCP>     } else if (type == LocalTime.class) {
//JCP>       if (sqlType == Types.TIME) {
//JCP>         Time timeValue = getTime(columnIndex);
//JCP>         if (wasNull()) {
//JCP>           return null;
//JCP>         }
//JCP>         return type.cast(timeValue.toLocalTime());
//JCP>       } else {
//JCP>         throw new SQLException("conversion to " + type + " from " + sqlType + " not supported");
//JCP>       }
//JCP>     } else if (type == LocalDateTime.class) {
//JCP>       if (sqlType == Types.TIMESTAMP) {
//JCP>         return type.cast(getLocalDateTime(columnIndex));
//JCP>       } else {
//JCP>         throw new SQLException("conversion to " + type + " from " + sqlType + " not supported");
//JCP>       }
//JCP>     } else if (type == OffsetDateTime.class) {
//JCP>       if (sqlType == Types.TIMESTAMP_WITH_TIMEZONE || sqlType == Types.TIMESTAMP) {
//JCP>         Timestamp timestampValue = getTimestamp(columnIndex);
//JCP>         if (wasNull()) {
//JCP>           return null;
//JCP>         }
//JCP>         // Postgres stores everything in UTC and does not keep original time zone
//JCP>         OffsetDateTime offsetDateTime = OffsetDateTime.ofInstant(timestampValue.toInstant(), ZoneOffset.UTC);
//JCP>         return type.cast(offsetDateTime);
//JCP>       } else {
//JCP>         throw new SQLException("conversion to " + type + " from " + sqlType + " not supported");
//JCP>       }
      //JCP! endif
  ...
  //JCP! if mvn.project.property.postgresql.jdbc.spec >= "JDBC4.2"
//JCP>   public void updateObject(int columnIndex, Object x, java.sql.SQLType targetSqlType,
//JCP>       int scaleOrLength) throws SQLException {
//JCP>     throw org.postgresql.Driver.notImplemented(this.getClass(), "updateObject");
//JCP>   }
//JCP> 
//JCP>   public void updateObject(String columnLabel, Object x, java.sql.SQLType targetSqlType,
//JCP>       int scaleOrLength) throws SQLException {
//JCP>     throw org.postgresql.Driver.notImplemented(this.getClass(), "updateObject");
//JCP>   }
//JCP> 
