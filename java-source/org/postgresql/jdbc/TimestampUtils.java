...
//JCP> import java.time.LocalDate;
//JCP> import java.time.LocalDateTime;
//JCP> import java.time.LocalTime;
//JCP> import java.time.OffsetDateTime;
//JCP> import java.time.ZoneOffset;
//JCP> import java.time.chrono.IsoEra;
//JCP> import java.time.temporal.ChronoField;
//JCP! endif
...
public class TimestampUtils {
  ...
  //JCP! if mvn.project.property.postgresql.jdbc.spec >= "JDBC4.2"
//JCP>   /**
//JCP>    * Parse a string and return a LocalDateTime representing its value.
//JCP>    *
//JCP>    * @param s The ISO formated date string to parse.
//JCP>    * @return null if s is null or a LocalDateTime of the parsed string s.
//JCP>    * @throws SQLException if there is a problem parsing s.
//JCP>    */
//JCP>   public LocalDateTime toLocalDateTime(String s) throws SQLException {
//JCP>     if (s == null) {
//JCP>       return null;
//JCP>     }
//JCP> 
//JCP>     int slen = s.length();
//JCP> 
//JCP>     // convert postgres's infinity values to internal infinity magic value
//JCP>     if (slen == 8 && s.equals("infinity")) {
//JCP>       return LocalDateTime.MAX;
//JCP>     }
//JCP> 
//JCP>     if (slen == 9 && s.equals("-infinity")) {
//JCP>       return LocalDateTime.MIN;
//JCP>     }
//JCP> 
//JCP>     ParsedTimestamp ts = parseBackendTimestamp(s);
//JCP> 
//JCP>     // intentionally ignore time zone
//JCP>     // 2004-10-19 10:23:54+03:00 is 2004-10-19 10:23:54 locally
//JCP>     LocalDateTime result = LocalDateTime.of(ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.nanos);
//JCP>     if (ts.era == GregorianCalendar.BC) {
//JCP>       return result.with(ChronoField.ERA, IsoEra.BCE.getValue());
//JCP>     } else {
//JCP>       return result;
//JCP>     }
//JCP>   }
  ...
  //JCP! if mvn.project.property.postgresql.jdbc.spec >= "JDBC4.2"
//JCP>   public synchronized String toString(LocalDate localDate) {
//JCP>     if (LocalDate.MAX.equals(localDate)) {
//JCP>       return "infinity";
//JCP>     } else if (LocalDate.MIN.equals(localDate)) {
//JCP>       return "-infinity";
//JCP>     }
//JCP> 
//JCP>     sbuf.setLength(0);
//JCP> 
//JCP>     appendDate(sbuf, localDate);
//JCP>     appendEra(sbuf, localDate);
//JCP> 
//JCP>     return sbuf.toString();
//JCP>   }
//JCP> 
//JCP>   public synchronized String toString(LocalTime localTime) {
//JCP>     if (LocalTime.MAX.equals(localTime)) {
//JCP>       return "infinity";
//JCP>     } else if (LocalTime.MIN.equals(localTime)) {
//JCP>       return "-infinity";
//JCP>     }
//JCP> 
//JCP>     sbuf.setLength(0);
//JCP> 
//JCP>     appendTime(sbuf, localTime);
//JCP> 
//JCP>     return sbuf.toString();
//JCP>   }
//JCP> 
//JCP> 
//JCP>   public synchronized String toString(OffsetDateTime offsetDateTime) {
//JCP>     if (OffsetDateTime.MAX.equals(offsetDateTime)) {
//JCP>       return "infinity";
//JCP>     } else if (OffsetDateTime.MIN.equals(offsetDateTime)) {
//JCP>       return "-infinity";
//JCP>     }
//JCP> 
//JCP>     sbuf.setLength(0);
//JCP> 
//JCP>     LocalDateTime localDateTime = offsetDateTime.toLocalDateTime();
//JCP>     LocalDate localDate = localDateTime.toLocalDate();
//JCP>     appendDate(sbuf, localDate);
//JCP>     sbuf.append(' ');
//JCP>     appendTime(sbuf, localDateTime.toLocalTime());
//JCP>     appendTimeZone(sbuf, offsetDateTime.getOffset());
//JCP>     appendEra(sbuf, localDate);
//JCP> 
//JCP>     return sbuf.toString();
//JCP>   }
//JCP> 
//JCP>   public synchronized String toString(LocalDateTime localDateTime) {
//JCP>     if (LocalDateTime.MAX.equals(localDateTime)) {
//JCP>       return "infinity";
//JCP>     } else if (LocalDateTime.MIN.equals(localDateTime)) {
//JCP>       return "-infinity";
//JCP>     }
//JCP> 
//JCP>     sbuf.setLength(0);
...
//JCP>   }
//JCP> 
//JCP>   private static void appendDate(StringBuilder sb, LocalDate localDate) {
//JCP>     int year = Math.abs(localDate.getYear()); // year is negative for BC dates
//JCP>     int month = localDate.getMonthValue();
//JCP>     int day = localDate.getDayOfMonth();
//JCP>     appendDate(sb, year, month, day);
//JCP>   }
//JCP> 
//JCP>   private static void appendTime(StringBuilder sb, LocalTime localTime) {
//JCP>     int hours = localTime.getHour();
//JCP>     int minutes = localTime.getMinute();
//JCP>     int seconds = localTime.getSecond();
//JCP>     int nanos = localTime.getNano();
//JCP>     appendTime(sb, hours, minutes, seconds, nanos);
//JCP>   }
//JCP> 
//JCP>   private void appendTimeZone(StringBuilder sb, ZoneOffset offset) {
//JCP>     int offsetSeconds = offset.getTotalSeconds();
//JCP> 
//JCP>     appendTimeZone(sb, offsetSeconds);
//JCP>   }
//JCP> 
//JCP>   private static void appendEra(StringBuilder sb, LocalDate localDate) {
//JCP>     if (localDate.get(ChronoField.ERA) == IsoEra.BCE.getValue()) {
//JCP>       sb.append(" BC");
//JCP>     }
//JCP>   }
  //JCP! endif
  ...
  //JCP! if mvn.project.property.postgresql.jdbc.spec >= "JDBC4.2"
//JCP>   /**
//JCP>    * Returns the local date time object matching the given bytes with {@link Oid#TIMESTAMP} or
//JCP>    * {@link Oid#TIMESTAMPTZ}.
//JCP>    *
//JCP>    * @param tz time zone to use
//JCP>    * @param bytes The binary encoded local date time value.
//JCP>    * @return The parsed local date time object.
//JCP>    * @throws PSQLException If binary format could not be parsed.
//JCP>    */
//JCP>   public LocalDateTime toLocalDateTimeBin(TimeZone tz, byte[] bytes) throws PSQLException {
//JCP> 
//JCP>     ParsedBinaryTimestamp parsedTimestamp = this.toParsedTimestampBin(tz, bytes, true);
//JCP>     if (parsedTimestamp.infinity == Infinity.POSITIVE) {
//JCP>       return LocalDateTime.MAX;
//JCP>     } else if (parsedTimestamp.infinity == Infinity.NEGATIVE) {
//JCP>       return LocalDateTime.MAX;
//JCP>     }
//JCP> 
//JCP>     return LocalDateTime.ofEpochSecond(parsedTimestamp.millis / 1000L, parsedTimestamp.nanos, ZoneOffset.UTC);
//JCP>   }
  //JCP! endif
